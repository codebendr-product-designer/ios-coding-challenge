...
class CountriesUITests: XCTestCase {
    ...
    func testPopulationFormat() {
        ...
        let afghanPopulation = app.staticTexts["Afghanistan-Population-Label"]
        ...
        let chinaPopulation = app.staticTexts["China-Population-Label"]
        XCTAssertTrue(afghanPopulation.waitForExistence(timeout: kTimeOut))
        commas = chinaPopulation.label.filter({return $0 == ","}).count
        XCTAssertEqual(3, commas)
        
        let antarcticaPopulation = app.staticTexts["Antarctica-Population-Label"]
    ...
    func testCapitals() {
        ...
        let algeriaCapital = app.staticTexts["Algeria-Capital-Label"]
        XCTAssertTrue(algeriaCapital.waitForExistence(timeout: kTimeOut))
        XCTAssertEqual("Algiers", algeriaCapital.label)
        
        let andorraCapital = app.staticTexts["Andorra-Capital-Label"]
